...
def get_img_data(record, size=640):
    ...
    mean = [0.485, 0.456, 0.406] #coco数据集计算而来
...
def multithread_loader(datadir, batch_size= 10, mode='train'):
    ...
    def get_data(samples):
        ...
        for sample in samples:
            record = sample[0]
            img_size = sample[1]
            img, gt_bbox, gt_labels, im_shape = get_img_data(record, size=img_size)#增广
            batch_data.append((img, gt_bbox, gt_labels, im_shape))
        return make_array(batch_data)

    mapper = functools.partial(get_data, )

    return paddle.reader.xmap_readers(mapper, reader, 16, 10)
